...
#define UART_RX_IRQ_OFF(u) \
    do { \
      USART_ITConfig((u)->hw, USART_IT_RXNE, DISABLE); \
    } while (0);
#define UART_RX_IRQ_ON(u) \
    do { \
      USART_ITConfig((u)->hw, USART_IT_RXNE, ENABLE); \
    } while (0);
...
#define UART_TX_IRQ_ON(u) \
    do { \
      USART_ITConfig((u)->hw, USART_IT_TXE, ENABLE); \
    } while (0);
#else
#define UART_CHECK_RX(u) ((u)->SR & USART_SR_RXNE)
#define UART_CHECK_TX(u) ((u)->SR & USART_SR_TXE)
#define UART_CHECK_OR(u) ((u)->SR & USART_SR_ORE)
#define UART_RX_IRQ_OFF(u) (u)->hw->CR1 &= ~USART_CR1_RXNEIE
#define UART_RX_IRQ_ON(u)  (u)->hw->CR1 |= USART_CR1_RXNEIE
#define UART_TX_IRQ_OFF(u) (u)->hw->CR1 &= ~USART_CR1_TXEIE
#define UART_TX_IRQ_ON(u)  (u)->hw->CR1 |= USART_CR1_TXEIE
#endif
...
void UART_irq(uart *u) {
  ...
  if ((UART_CHECK_RX(u->hw)) && (u->hw->CR1 & USART_CR1_RXNEIE)) {
    u8_t c = USART_ReceiveData(u->hw);
    u->rx.buf[u->rx.wix++] = c;
