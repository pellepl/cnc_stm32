...
__attribute__ (( noreturn )) void bootloader_execute() {
  ...
  print("\n\n**** Bootloader initialization from kernel\n\n");
  ...
  print("\n**** Kernel releasing command to bootloader in ram, bye... \n\n");
...
void bootloader_update_fw() {
  ...
  SHMEM_get()->user[BOOTLOADER_SHMEM_SPIF_FW_ADDR_UIX] = FIRMWARE_SPIF_ADDRESS;
}
